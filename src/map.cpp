...
{
    vehicle_part &vp_cargo = cur_veh.part( part );
    ...
    if( washer_here ) {
        ...
        for( item &n : cur_veh.get_items( vp_cargo ) ) {
    ...
    const bool autoclave_here = vp_cargo.enabled && cur_veh.part_flag( part, VPFLAG_AUTOCLAVE );
    if( autoclave_here ) {
